...
BinaryHeap.prototype.insert = function (value) {
  ...
  this.arrangeNodes(this._heap.length - 1);
}

BinaryHeap.prototype.arrangeNodes = function(pos){
  if(pos <= 0){
    return;
  } 
  var parent = Math.floor( (pos - 1) / 2 );
  if(this._compare(this._heap[pos],this._heap[parent])){
    this.swap(pos,parent);
    this.arrangeNodes(parent);
  }
}

BinaryHeap.prototype.swap = function(first,second){
  var temp = this._heap[first];
  this._heap[first] = this._heap[second];
  this._heap[second] = temp;
}

BinaryHeap.prototype.arrangeNodesFromRoot = function(pos){
  var left = 2 * pos + 1;
  var right = 2 * pos + 2;
  var largest = pos;
  if (left < this._heap.length && this._compare(this._heap[left], this._heap[largest])) {
    largest = left;
  }
  if (right < this._heap.length && this._compare(this._heap[right], this._heap[largest])) {
    largest = right;
  }
  if (largest != pos) {
    swap(largest, pos);
    this.arrangeNodesFromRoot(largest);
  }
}
