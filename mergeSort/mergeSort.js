...
var mergeSort = function(array) {
  // Your code here.
  var temp = [];
  mergesortR(0,array.length-1,temp,array);
  return array;
};

var mergesortR = function(start,end,temp,array){
  var mid = parseInt((end - start)/2 + start);
  if(end-start >= 1){
    mergesortR(start,mid,temp,array);
    mergesortR(mid+1,end,temp,array);
    merge(start,mid,end,temp,array);
  }
}

var merge = function(start, mid, end, temp,array) {
  var i = start, j = mid + 1;
  var k = 0;
  while(i <= mid && j <= end){
    if(array[i] < array[j]){
